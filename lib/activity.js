...
export function createActivityDetector(config) {
  const { dwell_threshold_minutes, walking_max_kmh, driving_min_kmh, window_size, max_accuracy_m = 30 } = config;
  let window = [];
  ...
  return {
    update(lat, lon, timestamp, vel, acc) {
      if (typeof acc === 'number' && acc > max_accuracy_m) {
        return { changed: false, state: currentState, previousState: currentState, initialClassification: false, gapTransition: null };
      }
