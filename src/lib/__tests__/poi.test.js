...
const JUST_OUTSIDE = { lat: 34.018079, lon: -117.9025897 }; // ~110m from Home (outside 100m, inside 125m)
const BEYOND_EXIT = { lat: 34.018259, lon: -117.9025897 };  // ~130m from Home (outside 125m)
...
describe('POI hysteresis (exit_extra_m)', () => {
  function makeHysteresisConfig(locations, exitExtra, defaultRadius = 100) {
    return {
      poi: { default_radius_m: defaultRadius, exit_extra_m: exitExtra, locations },
    };
  }

  it('point just outside entry radius does not trigger departure when within exit radius', () => {
    const detector = createPOIDetector(makeHysteresisConfig([HOME], 25));
    detector.setLocation('Home');
    // JUST_OUTSIDE is ~110m: outside 100m entry radius but inside 125m exit radius
    const r = detector.detect(JUST_OUTSIDE.lat, JUST_OUTSIDE.lon);
    assert.equal(r.changed, false);
    assert.equal(r.location, 'Home');
  });

  it('point beyond exit radius triggers departure', () => {
    const detector = createPOIDetector(makeHysteresisConfig([HOME], 25));
    detector.setLocation('Home');
    // BEYOND_EXIT is ~130m: outside 125m exit radius
    const r = detector.detect(BEYOND_EXIT.lat, BEYOND_EXIT.lon);
    assert.equal(r.changed, true);
    assert.equal(r.location, 'Roaming');
  });

  it('arrival still uses normal entry radius', () => {
    const detector = createPOIDetector(makeHysteresisConfig([HOME], 25));
    // Start roaming, arrive within entry radius
    const r = detector.detect(NEAR_HOME.lat, NEAR_HOME.lon);
    assert.equal(r.changed, true);
    assert.equal(r.location, 'Home');
  });

  it('point in hysteresis zone does not trigger arrival when roaming', () => {
    const detector = createPOIDetector(makeHysteresisConfig([HOME], 25));
    // JUST_OUTSIDE is ~110m: outside 100m entry radius
    const r = detector.detect(JUST_OUTSIDE.lat, JUST_OUTSIDE.lon);
    assert.equal(r.changed, false);
    assert.equal(r.location, 'Roaming');
  });

  it('defaults to exit_extra_m=0 when not configured', () => {
    const detector = createPOIDetector(makeConfig([HOME]));
    detector.setLocation('Home');
    // Without hysteresis, JUST_OUTSIDE (~110m) is outside 100m → departure
    const r = detector.detect(JUST_OUTSIDE.lat, JUST_OUTSIDE.lon);
    assert.equal(r.changed, true);
    assert.equal(r.location, 'Roaming');
  });
